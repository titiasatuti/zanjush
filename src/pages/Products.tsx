...
const Products = () => {
  ...
  const create = async () => {
    ...
    const { error } = await supabase.from("items").insert({ type, name, sku: finalSku, unit, min_stock: minStock, photo_url: photoUrl || null, is_active: true });
    if (error) return showError(error.message);
    showSuccess("Item created");
    setName(""); setCode(""); setPhotoUrl(""); setMinStock(0);
    refresh();
  };
    ...
    <AppLayout title="Products & Ingredients">
      <div className="grid gap-4 rounded-2xl border bg-white p-4">
        <div className="flex gap-2">
          <Button variant={type === "product" ? "default" : "secondary"} className="rounded-xl" onClick={() => setType("product")}>Product</Button>
          <Button variant={type === "ingredient" ? "default" : "secondary"} className="rounded-xl" onClick={() => setType("ingredient")}>Ingredient</Button>
        </div>
        ...
        <div>
          ...
          <div className="flex gap-2">
            ...
            <Button variant="secondary" className="rounded-xl" onClick={() => setCode(sku(name || "ITEM", type))}>Regenerate</Button>
          </div>
        </div>
        <div>
        ...
        <div><Label>Photo URL</Label><Input value={photoUrl} onChange={(e) => setPhotoUrl(e.target.value)} /></div>
        <Button className="rounded-xl bg-emerald-500 hover:bg-emerald-600" onClick={create}>Save</Button>
      </div>
