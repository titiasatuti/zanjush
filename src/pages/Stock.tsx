...
const Stock = () => {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      if (movementsRes.error || itemsRes.error) {
        setErrorText(movementsRes.error?.message || itemsRes.error?.message || "Gagal memuat ledger stok");
        setIsLoading(false);
        return;
      }
  ...
  return (
    <AppLayout title="Ledger Stok" backTo="/products">
      ...
      {isLoading ? (
        <div className="rounded-3xl border bg-white p-5 text-sm text-slate-500 shadow-sm">Memuat ledger stok...</div>
      ) : errorText ? (
